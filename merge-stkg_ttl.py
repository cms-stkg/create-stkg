...
from typing import Dict, Optional, Set, Tuple, List, DefaultDict
...
STKGID = Namespace("http://example.org/stkg/id/")
...
REL_SEED_TO_CANONICAL = {
    ...
    "next_to": CANONICAL_REL["side"],
}

def first_obj(graph: Graph, s: URIRef, p: URIRef):
    for o in graph.objects(s, p):
        return o
    return None

def all_objs(graph: Graph, s: URIRef, p: URIRef):
    return list(graph.objects(s, p))
...
def main():
    ...
    ap.add_argument("--kg1", required=True, help="path to KG1 (.ttl)")
    ap.add_argument("--kg2", required=True, help="path to KG2 (.ttl)")
    ap.add_argument("--out", required=True, help="output path (.ttl)")
    ...
    for g in (g1, g2):
        for s in g.subjects(RDF.type, STKGID.Platform):
            platform_uris.add(s)

    # original platform URI -> canonical entity URI
    ...
    for g in (g1, g2):
        for o in g.objects(None, STKG.relationType):
    ...
    for orig, canon in uri_to_canon.items():
        merged_g.add((canon, OWL.sameAs, orig))
    ...
    def collect_from(g: Graph):
        # --- PositionObservations ---
        for obs in g.subjects(RDF.type, STKGID.PositionObservation):
            ent0 = first_obj(g, obs, STKG.observedEntity)
            t0   = first_obj(g, obs, STKG.time)
            lat0 = first_obj(g, obs, GEO.lat)
            lon0 = first_obj(g, obs, GEO.long)

            sfiles = set(str(x) for x in all_objs(g, obs, STKG.sourceFile))
            srows  = set(str(x) for x in all_objs(g, obs, STKG.sourceRow))
            ...
            if args.pos_dedup == "strict":
                ...
                if key not in seen_pos_strict:
                    ...
                    merged_g.add((canon_obs, RDF.type, STKGID.PositionObservation))
        ...
        for robs in g.subjects(RDF.type, STKGID.SpatialRelationObservation):
            sub0 = first_obj(g, robs, STKG.subjectEntity)
            obj0 = first_obj(g, robs, STKG.objectEntity)
            rel0 = first_obj(g, robs, STKG.relationType)
            t0   = first_obj(g, robs, STKG.time)
            ...
            sfiles = set(str(x) for x in all_objs(g, robs, STKG.sourceFile))
            srows  = set(str(x) for x in all_objs(g, robs, STKG.sourceRow))
            ...
            if key not in seen_rel:
                ...
                merged_g.add((canon_robs, RDF.type, STKGID.SpatialRelationObservation))
    ...
    # ---- Finalize by_time PositionObservations ----
