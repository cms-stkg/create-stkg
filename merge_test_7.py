...
def build_entity_alignment_map_from_matches(position_records, matched_pairs, min_evidence=3):
    ...
    # evidence 순으로 strong pair부터 union
    # 현재는 min_evidence 이상이면 union
    sorted_pairs = sorted(
        pair_counter.items(),
...
def build_merged_position_items(all_position_records, representative_map):
    ...
    for rec in all_position_records:
        ...
        aligned_ent = representative_map.get(orig_ent, canonicalize_entity_name(rec["observed_entity"]) or rec["observed_entity"])
...
def merge_graphs(kg1_file: str, kg2_file: str, out_file: str):
    ...
    # 2) 술어 정규화는 extract_relation_observations에서 이미 수행됨
    # 3) 시공간 기준 위치 매칭
